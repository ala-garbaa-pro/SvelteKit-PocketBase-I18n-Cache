...
export const cookiesDeprotector = async (encryptedCookies: string) => {
	...
	const extractedEncryptedData = extractDataFromCookies(encryptedCookies);

	if (extractedEncryptedData !== null) {
		const decryptDataPart = await decryptText(extractedEncryptedData);

		const prefix = encryptedCookies.substring(0, encryptedCookies.indexOf('=') + 1);

		const suffix = encryptedCookies.substring(encryptedCookies.indexOf('; Path='));

		const fullString = constructCookies(decryptDataPart, prefix, suffix);
